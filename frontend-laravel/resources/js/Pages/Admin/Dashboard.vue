...
<template>
    <AppLayout>
            ...
            <div class="container">
                        ...
                        <div v-if="activeTab === 'dashboard'">
                            ...
                            <div class="table-card mt-6">
                                <div class="table-header">
                                    <h3 style="font-size:18px; margin: 0;">Daftar Pelanggan Terbaru</h3>
                                </div>
                                <div class="table-responsive">
                                    <table class="data-table">
                                        ...
                                        <tbody>
                                            <tr v-for="cust in (customers || []).slice(0, 5)" :key="cust.username">
                                                <td style="font-weight: 700;">{{ cust.name }}</td>
                                                <td>{{ cust.username }}</td>
                                                <td>{{ cust.email || '-' }}</td>
                                                <td>{{ cust.phone || '-' }}</td>
                                            </tr>
                                            <tr v-if="!(customers && customers.length)">
                                ...
                                <div style="padding: 16px; border-top: 1px solid #e2e8f0; text-align: center;">
                                    <button @click="activeTab = 'customers'" style="background: none; border: none; color: #e11d48; font-weight: 700; cursor: pointer; font-size: 14px;">Lihat Semua Pelanggan →</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                <tr v-for="order in filteredOrders" :key="order.id" @click="openOrderDetail(order)" style="cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
                                    <td style="font-weight: 800; font-family: monospace; color: #0f172a;">{{ order.id }}</td>
                ...
                <div v-if="activeTab === 'customers'" class="table-card">
                    <div class="table-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
                        ...
                        <div style="display: flex; align-items: center; background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 4px 12px; flex: 1; max-width: 400px;">
                            ...
                            <input 
                                ...
                                style="border: none; outline: none; padding: 8px; font-size: 14px; width: 100%;"
                            >
                        </div>
                    </div>
                    <div class="table-responsive">
        ...
        <!-- Order Detail Modal -->
        <div v-if="isOrderDetailOpen" class="d-flex" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:9999; align-items:center; justify-content:center; padding:16px;">
            <div style="width:100%; max-width:800px; max-height:90vh; display:flex; flex-direction:column; background: white; border-radius: 12px; overflow: hidden; animation: slideUp 0.3s forwards;">
                <!-- Header -->
                <div style="padding: 20px 24px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center; background: #f8fafc;">
                    <div>
                        <h3 style="font-size: 18px; font-weight: 700; color: #0f172a; margin: 0;">Detail Pesanan</h3>
                        <div style="font-size: 13px; color: #64748b; margin-top: 4px; font-family: monospace;">{{ selectedOrderDetail?.id }}</div>
                    </div>
                    <button @click="isOrderDetailOpen = false" style="background: none; border: none; font-size: 24px; color: #94a3b8; cursor: pointer;">&times;</button>
                </div>
